...
async function main() {
  ...
  Object.values(results).forEach(result => {
    ...
    console.log(`| ${result.description} | ${duration} | ${avgLoadTime}ms | ${throttlingStatus} | ${passRate} |`);
  });
  ...
  // Compare local modes
  if (results.headed.metrics.loadTimes.length > 0 && results.headless.metrics.loadTimes.length > 0) {
    const headedAvg = results.headed.metrics.loadTimes.reduce((a, b) => a + b, 0) / results.headed.metrics.loadTimes.length;
    const headlessAvg = results.headless.metrics.loadTimes.reduce((a, b) => a + b, 0) / results.headless.metrics.loadTimes.length;
  ...
  if (results.headless.metrics.loadTimes.length > 0 && results.flake.metrics.loadTimes.length > 0) {
    const headlessAvg = results.headless.metrics.loadTimes.reduce((a, b) => a + b, 0) / results.headless.metrics.loadTimes.length;
    const flakeAvg = results.flake.metrics.loadTimes.reduce((a, b) => a + b, 0) / results.flake.metrics.loadTimes.length;
  ...
  console.log(`• Local Headed Success Rate: ${results.headed.metrics.testResults.passed}/${results.headed.metrics.testResults.passed + results.headed.metrics.testResults.failed} tests passed`);
  console.log(`• Local Headless Success Rate: ${results.headless.metrics.testResults.passed}/${results.headless.metrics.testResults.passed + results.headless.metrics.testResults.failed} tests passed`);
  console.log(`• Simulated CI Success Rate: ${results.flake.metrics.testResults.passed}/${results.flake.metrics.testResults.passed + results.flake.metrics.testResults.failed} tests passed`);
  
  if (results.ci.metrics.testResults.passed + results.ci.metrics.testResults.failed > 0) {
    console.log(`• Actual CI Success Rate: ${results.ci.metrics.testResults.passed}/${results.ci.metrics.testResults.passed + results.ci.metrics.testResults.failed} tests passed`);
  }
  
  if (results.flake.metrics.throttlingApplied) {
    console.log('• Throttling: Successfully applied CPU and network throttling in simulated CI mode');
  } else {
